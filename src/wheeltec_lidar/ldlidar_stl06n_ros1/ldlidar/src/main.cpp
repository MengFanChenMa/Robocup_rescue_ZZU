...
int main(int argc, char **argv) {
  ...
  nh_private.param("min_range", setting.min_range, double(0.3));
  nh_private.param("max_range", setting.max_range, double(20.0));
  nh_private.param("flag_parted", setting.flag_parted, false);
  nh_private.param<std::vector<int>>("disable_min", setting.disable_angle_min_range, {0});
  nh_private.param<std::vector<int>>("disable_max", setting.disable_angle_max_range, {0});
  ...
  ROS_INFO("<min_range>: %f", setting.min_range);
  ROS_INFO("<max_range>: %f", setting.max_range);
...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src, double lidar_spin_freq, 
  ...
  if (lidar_spin_freq > 0) {
    ...
    for (auto point : src) {
      ...
      float dir_angle = point.angle+90;
      if(dir_angle>360)dir_angle= dir_angle-360;
      ...
      if(setting.flag_parted){
            if(setting.disable_angle_min_range.size()==setting.disable_angle_max_range.size())
              {
              for(int j=0;j<setting.disable_angle_min_range.size();++j)
              {
                  if(setting.disable_angle_min_range[j]<setting.disable_angle_max_range[j]){
                  if((index>=(setting.disable_angle_min_range[j]*beam_size / 360))&&(index<=(setting.disable_angle_max_range[j]*beam_size / 360))){
                    range = std::numeric_limits<float>::quiet_NaN();
                    intensity = std::numeric_limits<float>::quiet_NaN();
                    }
                  }
